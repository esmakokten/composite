...
set_next_timer_interrupt(struct slm_thd *t, cycles_t now)
{
	...
	/* Are there any thread inAnna Lyons timer queue? */
...
slm_sched_fpss_schedule(cycles_t now)
{
	...
	for (i = 0 ; i < SLM_FPRR_NPRIOS ; i++) {
		...
		//COS_TRACE("\"event\":\"schedule\", \"tid\":%ld, \"next-timeout\":%llu, \"iter\":%d", slm_thd_from_sched(st)->tid, g->current_timeout, i);
...
#endif
			//COS_TRACE("\"event\":\"replenish\", \"tid\":%ld, \"amount\":%llu, \"planned_time\":%llu", t->tid, st->replenishments[st->repl_head_idx].replenish_amount, st->replenishments[st->repl_head_idx].replenish_time_abs);
...
slm_sched_fpss_wakeup(struct slm_thd *t)
{
	...
	//assert(tt->timeout_idx == -1);
	if(tt->timeout_idx != -1) {
		COS_TRACE("\"event\":\"#ASSERT#\", \"tid\":%ld, \"timeout-idx\":%d, current thd: %ld", t->tid, tt->timeout_idx, cos_thdid());
		//assert(0);
		slm_timer_fpss_cancel(t);
	}
...
slm_sched_fpss_init(void)
{
	...
	printc("### SLM FPSS Scheduler Initialized ###\n");
}
