...
{
	// Process the timers of the higher or equal priority
	//for (int i = SLM_FPRR_PRIO_HIGHEST; i <= prio; i++) {
...
slm_sched_fpres_execution(struct slm_thd *t, cycles_t cycles, cycles_t now)
{
	...
	*/
	...
	if (st->state == STATE_BLOCKED || st->state == STATE_BLOCKED_PERIODIC) {
		...
		COS_TRACE("#############Skipping adding timer for blocked thread",0,0,0);
		//assert(0);
		return;
	}
...
slm_sched_fpres_schedule(cycles_t now)
{
	...
	for (i = 0 ; i < SLM_FPRR_NPRIOS ; i++) {
		...
		if ((tt->is_budgeted) && (tt->budget <= 0)) {
			COS_TRACE("#########(ASSERT): TID: %ld Budget: %lld\n", slm_thd_from_sched(st)->tid, tt->budget, 0);
			//printc("####print#####(ASSERT): TID: %ld Budget: %lld\n", slm_thd_from_sched(st)->tid, tt->budget,0);
			continue;
		}	
		...
		if (st->repl_count > 0 && (st->repl_count != SLM_FPRES_REPL_WINDOW_SIZE) && (g_prev_tid == slm_thd_from_sched(st)->tid)) {
			assert(0);
			...
			if (tt->timeout_idx != -1 && (tt->abs_next_processing == st->replenishments[st->repl_tail_idx].replenish_time_abs)) {
				...
				slm_timer_fpres_cancel(slm_thd_from_sched(st));
			}
			st->repl_count--;
		}
...
#define EAGER_REPLENISHMENT
		...
		/* Check if closest_prior_timer is already processed */
		if (next_processing_thd && slm_thd_from_sched(st)->tid == slm_thd_from_timer(next_processing_thd)->tid){
			struct timer_global *g = timer_global_prio(i);
			closest_prior_timer = 0;
			if (heap_size(&g->h) > 0) {
				closest_prior_timer = slm_thd_timer_policy(heap_peek(&g->h))->abs_next_processing;
			}
			closest_prior_timer = ((closest_prior_timer == 0) || (prev_closest_prior_timer < closest_prior_timer)) ? prev_closest_prior_timer : closest_prior_timer;
		}
