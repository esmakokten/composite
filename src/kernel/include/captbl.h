...
captbl_add(struct captbl *t, capid_t cap, cap_t type, int *retval)
{
	...
	if (unlikely(sz == CAP_SZ_ERR)) {
		printk("captbl_add: invalid cap type %d\n", (int)type);
	  	cos_throw(err, -EINVAL);
	}
	if (unlikely(cap >= __captbl_maxid())) {
		printk("captbl_add: cap %d exceeds max cap id\n", (int)cap);
	  	cos_throw(err, -EINVAL);
	}
	...
	if (unlikely(__captbl_header_validate(&l, sz))){
		printk("captbl_add: header validation failed for cap %d, type %d\n", (int)cap, (int)type);
		cos_throw(err, -EINVAL);
	}
